...
# include "allocate.h"
  ...
  public :: getImgRange, getSuperSampling, printMatrix
  ...
  ! routines to print sparse matrices to file
  interface printMatrix
    module procedure sparse_realprint
    module procedure sparse_cmplxprint
  end interface
...
contains
  ...
  subroutine getCellTranslations(cellVec, rCellVec, latVec, recVec2p, cutoff)
    ...
    call getLatticePoints(cellVec, latVec, recVec2p, cutoff, posExtension=1, &
        &negExtension=1)
    INITALLOCATE_PARR(rCellVec, (3, size(cellVec, dim=2)))
  ...
  subroutine getLatticePoints(latPoint, latVec, recVec2p, dist, posExtension, &
    ...
    INITALLOCATE_PARR(tmpLatPoint, \
        (3, product(sum(abs(imgRange(:,:)), dim=1)+1)))
    ...
    if (ind == size(tmpLatPoint, dim=2)) then
      latPoint => tmpLatPoint
    else
      INITALLOCATE_PARR(latPoint, (3, ind))
      latPoint(:,:) = tmpLatPoint(:,:ind)
      DEALLOCATE_PARR(tmpLatPoint)
    end if
  ...
  !!* @param specie Specie of all interacting atoms on exit.
  ...
  !!* @param specie0 Species of the atoms in the central cell
  !!* @param cutoff Cutoff until neighborlist should be created
  !!* @param rCellVec Cell vector for the translated cells to consider.
  subroutine updateNeighborListAndSpecies(coord, specie, img2CentCell, &
      &iCellVec, neigh, nAllAtom, coord0, specie0, cutoff, rCellVec)
    ...
    if (size(specie) < nAllAtom) then
      DEALLOCATE_PARR(specie)
      ALLOCATE_PARR(specie, (nAllAtom))
    end if
    specie(1:nAllAtom) = specie0(img2CentCell(1:nAllAtom))
  ...
  subroutine updateNeighborList(coord, img2CentCell, iCellVec, neigh, &
    ...
    !! Clean arrays.
    !! (Every atom is the 0th neigbhor of itself with zero distance square.)
    neigh%nNeighbor(:) = 0
    neigh%iNeighbor(:,:) = 0
    do ii = 1, nAtom
...
99010 format ('Cutoff (', E16.6, ') greater then last cutoff ', &
  ...
  !!* @param iSquare  Returns the offset array for each atom.
  !!* @param orb Information about the orbitals in the system.
  subroutine buildSquaredAtomIndex(iSquare, orb)
    ...
    ASSERT(all(shape(iSquare) == (/ nAtom + 1 /)))
  ...
  subroutine getSuperSampling(coeffs, shifts, latVecs, recVecs2p, kPoints, &
    ...
    logical :: tReduce
    real(dp) :: superLatVecs(3,3), superRecVecs(3,3)
    real(dp) :: invCoeffs(3,3), rr(3)
    real(dp) :: mDist2, dist2
    integer :: imgRange(2,3)
    ...
    superLatVecs(:,:) = matmul(latVecs, coeffs)
    superRecVecs(:,:) = superLatVecs(:,:)
    call matinv(superRecVecs)
    superRecVecs(:,:) = reshape(superRecVecs, (/3, 3/), order=(/2, 1/))  ! B
    invCoeffs(:,:) = coeffs(:,:)
    call matinv(invCoeffs)
    invCoeffs = reshape(invCoeffs, (/3, 3 /), order=(/2,1/)) !n^-1^t
    mDist2 = 0.0_dp
    do i1 = 0, 1
      do i2 = 0, 1
        do i3 = 0, 1
          dist2 = sum(matmul(recVecs2p, real((/ i1, i2, i3 /), dp))**2)
          if (dist2 > mDist2) then
            mDist2 = dist2
          end if
        end do
      end do
    end do
    !! To make sure, the shifts do not cause problems, we take one shell
    !! more then really necessary
    call getImgRange(imgRange, sqrt(mDist2), superLatVecs, 1, 1)
    call init(lr1)
    do i1 = imgRange(1, 1), imgRange(2, 1)
      do i2 = imgRange(1, 2), imgRange(2, 2)
        do i3 = imgRange(1, 3), imgRange(2, 3)
          rr(:) = matmul(invCoeffs, real((/ i1, i2, i3 /), dp) + shifts)
    ...
    nAllKPoint = len(lr1)
    if (abs(real(nAllKPoint,dp) - determinant33(coeffs)) > tol) then
  ...
  !!* @param fdOverlap    File ID to print to
  ...
  !!* @param iSquare      Atom offset for the squared Hamiltonian
  !!* @param iPair indexing array for the sparse Hamiltonian
  !!* @param img2CentCell Atomic mapping indexes.
  !!* @param orb Information about the orbitals in the system
  subroutine sparse_cmplxprint(fdOverlap, orig, kPoint, iNeighbor, &
      & nNeighbor, iCellVec, cellVec, iSquare, iPair, img2CentCell, orb)
    integer,      intent(in) :: fdOverlap
    ...
    do iAtom1 = 1, nAtom
      ...
      do iNeigh = 0, nNeighbor(iAtom1)
        ...
        tmp(1:nOrb2,1:nOrb1) = phase * &
            & reshape(orig(iOrig:iOrig+nOrb1*nOrb2-1), (/nOrb2, nOrb1/))
        if (iNeigh == 0) then
          do ll = 1, nOrb1
            do kk = ll, nOrb2
              write(fdOverlap,*)kk+jj-1,ll+ii-1, tmp(kk,ll)
            end do
          end do
        else
          do ll = 1, nOrb1
  ...
  end subroutine sparse_cmplxprint
  ...
  !!* @param img2CentCell Atomic mapping indexes.
  !!* @param orb Information about the orbitals in the system
  subroutine sparse_realprint(fdOverlap, orig, iNeighbor, nNeighbor, &
      & iSquare, iPair, img2CentCell,orb)
    integer,  intent(in)  :: fdOverlap
    real(dp), intent(in)  :: orig(:)
    integer,  intent(in)  :: iNeighbor(0:,:)
    integer,  intent(in)  :: nNeighbor(:)
  ...
  end subroutine sparse_realprint
