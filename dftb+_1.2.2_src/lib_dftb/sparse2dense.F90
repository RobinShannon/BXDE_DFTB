...
!    module procedure blockAntiSymmetrizeHS_cmplx
  end interface
...
contains
  ...
  subroutine unpackHS_cmplx(square, orig, kPoint, iNeighbor, nNeighbor, &
      &iCellVec, cellVec, iSquare, iPair, img2CentCell)
    ...
    ASSERT(size(square, dim=1) == iSquare(nAtom+1) - 1)
  ...
  !!*   of the matrix. To fill the matrix completely, apply the
  !!*   blockSymmetrizeHS subroutine.
  subroutine unpackHS_real(square, orig, iNeighbor, nNeighbor, iSquare, &
      &iPair, img2CentCell)
    real(dp), intent(out) :: square(:,:)
    real(dp), intent(in)  :: orig(:)
    integer,  intent(in)  :: iNeighbor(0:,:)
  ...
  !!* @param iCellVec     Index of the cell translation vector for each atom.
  !!* @param cellVec      Relative coordinates of the cell translation vectors.
  !!* @param iSquare      Atom offset for the squared matrix
  !!* @param iPair indexing array for the sparse Hamiltonian
  ...
  subroutine packHS_cmplx(primitive, square, kPoint, kWeight, iNeighbor, &
      & nNeighbor, mOrb, iCellVec, cellVec, iSquare, iPair, img2CentCell)
  ...
  subroutine packHSPauli(primitive, square, iNeighbor, nNeighbor, mOrb, &
    ...
    nAtom = size(iNeighbor, dim=2)
    nOrb = (iSquare(nAtom+1) - 1) ! number of orbitals in a regular spin block
  ...
  subroutine packHSPauliImag_kpts(primitive, square, kPoint, kWeight, &
      & iNeighbor, nNeighbor, mOrb, iCellVec, cellVec, iSquare, iPair, &
      & img2CentCell)
    real(dp), intent(inout) :: primitive(:,:)
    complex(dp), intent(in) :: square(:,:)
    real(dp),    intent(in) :: kPoint(:)
  ...
  subroutine blockSymmetrizeHS_cmplx(square, iSquare)
  ...
  !!* @param square   Square form matrix.
  !!* @param iSquare  Returns the offset array for each atom.
  subroutine blockHermitianHS_cmplx(square, iSquare)
    complex(dp), intent(inout) :: square(:,:)
    integer, intent(in) :: iSquare(:)
  ...
  !!* @param square   Square form matrix.
  !!* @param iSquare  Returns the offset array for each atom.
  subroutine blockSymmetrizeHS_real(square, iSquare)
  ...
  !!* (Real version)
  !!* @param square   Square form matrix.
  !!* @param iSquare  Contains the offset in the array for each atom.
  subroutine blockAntiSymmetrizeHS_real(square, iSquare)
    real(dp), intent(inout) :: square(:,:)
    integer,  intent(in)    :: iSquare(:)
  ...
  !!* @param square   Square form matrix.
  !!* @param iSquare  Contains the offset in the array for each atom.
  subroutine blockAntiSymmetrizeHS_cmplx(square, iSquare)
    complex(dp), intent(inout) :: square(:,:)
    integer,  intent(in)       :: iSquare(:)
  ...
  end subroutine blockAntiSymmetrizeHS_cmplx
