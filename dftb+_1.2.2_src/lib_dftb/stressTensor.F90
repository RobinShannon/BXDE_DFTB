...
#include "allocate.h"
  ...
  public :: getRepulsiveStress, getVirialStress, getNonSCCStress,&
...
contains
  ...
  !!* The virial contribution to the stress tensor
  !!* @param st stress tensor
  !!* @param mass particle masses
  !!* @param species Species of atoms in the central cell.
  !!* @param velo particle velocities
  !!* @param cellVol cell volume.
  subroutine getVirialStress(st, mass, species, velo, cellVol)
    real(dp), intent(out) :: st(:,:)
    ...
    ASSERT(size(mass) == maxval(species))
    ...
    do iAtom = 1, nAtom
      do ii = 1, 3
        do jj = 1, 3
          st(jj,ii) = st(jj,ii) &
              &+ mass(species(iAtom)) * velo(jj,iAtom) * velo(ii,iAtom)
        end do
      end do
    end do
  ...
  end subroutine getVirialStress
  ...
  !!* @param cellVol cell volume.
  subroutine getNonSCCStress(st, DM, EDM, skHamCont, skOverCont, coords, &
      & species, iNeighbor, nNeighbor, img2CentCell, iPair, orb, cellVol)
    real(dp), intent(out) :: st(:,:)
  ...
  !!* @param cellVol cell volume.
  subroutine getBlockStress(st, DM, EDM, skHamCont, skOverCont, coords, &
      &species, iNeighbor, nNeighbor, img2CentCell, iPair, orb, shift, cellVol)
    real(dp), intent(out) :: st(:,:)
    real(dp), intent(in) :: DM(:,:)
    real(dp), intent(in) :: EDM(:)
    type(OSlakoCont), intent(in) :: skHamCont, skOverCont
  ...
  !!* @param cellVol cell volume.
  subroutine getBlockiStress(st, DM, iDM, EDM, skHamCont, skOverCont, coords, &
