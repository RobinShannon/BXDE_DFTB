...
#include "assert.h"  
  ...
  public :: init_ExtChrg, destruct_ExtChrg
  ...
  real(dp), pointer :: rCellVec_(:,:)  ! Real lattice points for Ewald-sum.
...
contains
  ...
  subroutine init_ExtChrg(coordsAndCharges, nAtom, latVecs, recVecs, &
    ...
    real(dp), intent(in), optional :: latVecs(3,3)
    real(dp), intent(in), optional :: recVecs(3,3)
  ...
  !!* Destroys the calculator for the external charges.
  subroutine destruct_ExtChrg()

    ASSERT(tInitialized_)

    DEALLOCATE_(coords_)
    DEALLOCATE_(charges_)
    DEALLOCATE_(invRVec_)
    DEALLOCATE_PARR(rCellVec_)
    DEALLOCATE_(blurWidths_)
    
    tInitialized_ = .false.
    
  end subroutine destruct_ExtChrg
