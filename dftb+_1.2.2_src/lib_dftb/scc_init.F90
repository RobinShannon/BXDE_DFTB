...
contains
  ...
  !!* @param specie List of chemical species for each atom
  !!* @param specieNames  Names of the species (for error messages)
  !!* @param orb Information about the orbitals.
  subroutine initQFromAtomChrg(fOrb, qAtom, fRefShell, species, specieNames, orb)
    real(dp), intent(out) :: fOrb(:,:,:)
    real(dp), intent(in)  :: qAtom(:)
    real(dp), intent(in) :: fRefShell(:,:)
    integer,   intent(in) :: species(:)
    character(len=*), intent(in) :: specieNames(:)
    ...
    ASSERT(size(specieNames) == size(orb%nShell))
    ...
    do iAt = 1, nAtom
      ...
      if (fAtomRes > 1e-4_dp) then
        call error("Not enough orbitals on species '" // trim(specieNames(iSp)) &
  ...
  !!* atomic closed shell
  subroutine initQFromShellChrg(qq, qShell, specie, orb)
  ...
  subroutine initQFromFile(qq, fileName, nEl, orb, magnetisation, qBlock, &
      & qiBlock)
    real(dp), intent(out)          :: qq(:,:,:)
    ...
    ASSERT(present(magnetisation).eqv.(nSpin==2))
...
99000 format ('External file of charges has a total charge:', &
    ...
    if (present(magnetisation)) then
      ...
      if (abs(sumQ - magnetisation) >= 1e-3_dp) then
        write(error_string, 99010) sumQ, nEl
