...
# include "allocate.h"  
  ...
  public :: invR, sumInvR, addInvRPrime, getOptimalAlphaEwald, getMaxGEwald, &
      & getMaxREwald, ewald, invR_stress
...
#ifdef EXTERNALERFC
              fTmp = fTmp * extErf(dist/blurWidths1(iAt1))
#else              
              fTmp = fTmp * erf(dist/blurWidths1(iAt1))
#endif              
            end if
...
#ifdef EXTERNALERFC
      realSum = realSum +  extErfc(alpha*absRR)/absRR
#else
      realSum = realSum + erfc(alpha*absRR)/absRR
#endif
    end do
...
#ifdef EXTERNALERFC
      dewr(:) = dewr &
          &+ rNew(:) * (-2.0_dp/sqrt(pi)*exp(-(alpha*rr)**2)*alpha*rr &
          &- extErfc(alpha*rr))/(rr**3)
#else
      dewr(:) = dewr &
          &+ rNew(:) * (-2.0_dp/sqrt(pi)*exp(-alpha*alpha*rr*rr)* alpha*rr &
          &- erfc(alpha*rr))/(rr*rr*rr)
#endif
    end do
...
#ifdef EXTERNALERFC
    derivRTerm (:) = r(:) * &
        & (-2.0_dp/sqrt(pi)*exp(-(alpha*rr)**2)*alpha*rr &
        & - extErfc(alpha*rr))/(rr**3)
#else
    derivRTerm (:) = r(:)*(-2.0_dp/sqrt(pi)*exp(-alpha*alpha*rr*rr)* &
        & alpha*rr - erfc(alpha*rr))/(rr*rr*rr)
#endif
