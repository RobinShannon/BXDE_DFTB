...
!$OMP  END PARALLEL DO
      ...
      call herk(dm, eigenvecs(:,1:nLevels), beta=cmplx(shift,0.0,dp))
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(ii) SCHEDULE(RUNTIME)
      do ii = 1, nLevels
        eigenvecs(:,ii) = eigenvecs(:,ii) / sqrt(filling(ii)-shift)
      end do
...
!$OMP  END PARALLEL DO
      ...
      call herk(dm, eigenvecs(:,1:nLevels), &
          & alpha=sign(1.0_dp,maxval(fillProduct(1:nLevels)))*cmplx(1.0,0.0,dp))
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(ii) SCHEDULE(RUNTIME)
      do ii = 1, nLevels
        eigenvecs(:,ii) = eigenvecs(:,ii) / sqrt(abs(fillProduct(ii)))
      end do
...
!$OMP  END PARALLEL DO
  ...
  subroutine sp_density_matrix_real(dm, eigenvecs, filling, iNeighbor, &
      &nNeighbor, orb, iSquare, img2CentCell)
    real(dp), intent(out) :: dm(:,:)
...
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(iNeigh1,start2,nOrb2) SCHEDULE(RUNTIME)
      do iNeigh1 = 0, nInCellNeighbor(iAt1)
        start2 = iSquare(inCellNeighbor(iNeigh1,iAt1))
...
!$OMP  END PARALLEL DO
  ...
  subroutine sp_energy_density_matrix_real(dm, eigenvecs, filling, eigen, &
      &iNeighbor, nNeighbor, orb, iSquare, img2CentCell)
    real(dp), intent(out) :: dm(:,:)
    real(dp), intent(in) :: eigenvecs(:,:)
    real(dp), intent(in) :: filling(:)
    real(dp), intent(in) :: eigen(:)
