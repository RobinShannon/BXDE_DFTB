...
contains
  ...
  subroutine onsite(Eatom, rho, iAtomStart, xi, orb, species)
    ...
    do ii = 1, nAtom
      ...
      tmpBlock(1:kk,1:kk) = & ! two ud/du blocks so omit 0.5 factor
          ...
          & iAtomStart(ii):iAtomStart(ii+1)-1)
      Eatom(ii) = Eatom(ii) +sum(SpeciesPlus(1:kk,1:kk,jj) &
          & *conjg(tmpBlock(1:kk,1:kk)))
  ...
  subroutine dual(Eatom, qBlockSkew, xi, orb, species)
    ...
    do ii = 1, nAtom
      ...
      total = 0.0_dp
      Eatom(ii) = Eatom(ii) - sum( transpose(tmpBlock) * SpeciesL(:,:,3,iSp) )
      ...
      total = 0.0_dp
      Eatom(ii) = Eatom(ii) - sum( &
          & transpose(tmpBlock) * (i * SpeciesL(:,:,1,iSp)+ SpeciesL(:,:,2,iSp) ))
