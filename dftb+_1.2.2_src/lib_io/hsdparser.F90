...
contains
  ...
  subroutine parseHSD_file(initRootName, file, xmlDoc)
    ...
    open(fd, file=file, iostat=iostat, status='old', action='read')
    if (iostat /= 0) then
  ...
  !!* @param depth       Number of open blocks/assignements.
  !!* @param residual    Unparsed text from the previous line
  !!* @param tRightValue Is next parsed token a right value of an assignement?
  ...
  recursive function parse_recursive(curNode, depth, residual, tRightValue, &
    ...
    lpMain: do while ((.not. tTagClosed) .and. (.not. tFinished))
      ...
      case (7)
        if (tRightValue) then
          !! Single assignement is terminated by a semi-colon. Text after
          ...
          call parsingError("Invalid assignement separator", curFile, curLine)
        end if
      end select
      ...
      case(4)
        ...
        if (len_trim(word) == 0) then
          call parsingError("Missing field name on the lhs of the &
              &assignement.", curFile, curLine)
        elseif (len_trim(strLine) == 0) then
          call parsingError("Missing value on the rhs of the assignement.", &
              &curFile, curLine)
...
9991  format("HSD parser error: File '",A,"', Line",I5,".")
  ...
  !!* @param fd          File descriptor for an open file where output should go
  !!* @param tRightValue Is current node the right hand side of an assignement?
  ...
  recursive subroutine dumpHSD_recursive(node, indent, fd, tRightValue, buffer)
    ...
    if (tRightValue) then
      ...
      !! We are the left hand side of an assignement -> write assignement sign
