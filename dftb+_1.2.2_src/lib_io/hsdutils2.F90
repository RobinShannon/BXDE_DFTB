...
contains
  ...
  subroutine warnUnprocessedNodes(node, tIgnoreUnprocessed, nodeList)
    type(fnode), pointer               :: node
    logical, intent(in)                :: tIgnoreUnprocessed
    ...
    if (ll > 0) then
      msg = "The following " // i2c(ll) &
          &// " node(s) had been ignored by the parser:" // newline
      !! WORKAROUND: alpha-osf1-compaq
      !! The loop goes from 1 to ll instead of 0, to ll-1 to make sure the
      !! i2c() call contains only ii and not ii+1 as argument. i2c calls with
      !! operation or function calls crashes on the alphas in Pb. (compiler bug)
      do ii = 1, ll 
        child => item(list, ii-1)
        call append_to_string(msg, "(" // i2c(ii) // ")")
  ...
  !!* Creates an empty HSD (XML) document
  !!* @param document Pointer to the empty document at exit
  !!* @note This subroutine has the only purpose to hide pointer assignement.
  subroutine createHSD(document)
    type(fnode), pointer :: document

    document => createDocumentNode()

  end subroutine createHSD
