...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  !!* Converts a string containing atom indices, ranges and specie names to
  !!* a list of atom indeces.
  !!* @param str  String to convert
  !!* @param specieNames  Contains the valid species names.
  ...
  subroutine convAtomRangeToInt(str, specieNames, species, node, val)
    ...
    do while (iostat == TOKEN_OK)
      call process(char(buffer), specieNames, species, nAtom, node, li)
      call getNextToken(str, buffer, iStart, iostat)
    end do
  ...
  contains
    ...
    ! Helper routine.
    subroutine process(cbuffer, specieNames, species, nAtom, node, li)
      ...
      if ((cbuffer(1:1) >= "0" .and. cbuffer(1:1) <= "9") &
        ...
        iPos = 0
        do iSp = 1, size(specieNames)
          if (specieNames(iSp) == cbuffer) then
            iPos = iSp
            exit
          end if
        end do
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  !!* @param name  Name of the node
  !!* @param value Value of the node
  subroutine writeChVal_char(xf, name, value)
    type(xmlf_t), intent(inout) :: xf
    character(len=*), intent(in) :: name
    character(len=*), intent(in) :: value
    
    call writeChild_local(xf, name, value)

  end subroutine writeChVal_char
