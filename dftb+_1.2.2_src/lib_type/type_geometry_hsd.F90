...
# include "allocate.h"  
  ...
  use StringList
  use CharManip
  use flib_dom, flib_normalize => normalize
  use flib_wxml
  use LapackRoutines
  implicit none
  private
...
contains
  ...
  subroutine writeTGeometryHSD_dom(node, geo)
    ...
    call setChildValue(node, "TypeNames", geo%specieNames, .false.)
  ...
  subroutine writeTGeometryHSD_xmlf(xf, geo)
    ...
    call writeChildValue(xf, "TypeNames", geo%specieNames)
  ...
  subroutine readTGeometryHSD(node, geo)
    ...
    geo%nSpecie = len(stringBuffer)
    if (geo%nSpecie == 0) then
      call detailedError(node, "Missing specie names.")
    end if
    INITALLOCATE_PARR(geo%specieNames, (geo%nSpecie))
    call asArray(stringBuffer, geo%specieNames)
    ...
    if (any(geo%species < 1 .or. geo%species > geo%nSpecie)) then
    ...
    if (geo%tPeriodic) then
      ...
      geo%recVecs2p(:,:) = geo%latVecs(:,:)
      call matinv(geo%recVecs2p, iError=iErr)
      if (iErr /= 0) then
  ...
  subroutine readTGeometryGen_help(node, geo, text)
    ...
    integer :: ii, iTmp
    real(dp) :: coords(3), rTmp
    type(listString) :: specieNames
    ...
    !! Since we can't rely on line breaks, we try to read in integers. If it
    !! fails, we had a specie name, so it will be read as string.
    call init(specieNames)
    iErr = TOKEN_ERROR
    iOldStart = iStart
    ...
    do while (iErr /= TOKEN_OK)
      ...
      call append(specieNames, char(txt))
      iOldStart = iStart
      call getNextToken(text, iTmp, iStart, iErr)
    end do
    geo%nSpecie = len(specieNames)
    if (geo%nSpecie == 0) then
      call detailedError(node, "Number of species equals zero.")
    end if
    INITALLOCATE_PARR(geo%specieNames, (geo%nSpecie))
    call asArray(specieNames, geo%specieNames)
    call destroy(specieNames)

    !! Read in sequential and specie indices.
    INITALLOCATE_PARR(geo%species, (geo%nAtom))
    INITALLOCATE_PARR(geo%coords, (3, geo%nAtom))
    iStart = iOldStart
    do ii = 1, geo%nAtom
      ...
      call checkError(node, iErr, "Bad specie number for an atom.")
      call getNextToken(text, coords, iStart, iErr)
      call checkError(node, iErr, "Bad coordinates for an atom.")
      geo%coords(:, ii) = coords(:)
    end do
    if (geo%nSpecie /= maxval(geo%species) .or. minval(geo%species) /= 1) then
