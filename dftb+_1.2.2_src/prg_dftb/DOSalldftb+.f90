...
!  use EMFields
  ...
  character(len=*), parameter :: bxdFormat = "(A8,F16.8,I10)"
  character(len=*), parameter :: bxdFormat3 = "(A8,F16.10,F16.10,F16.10)"
...
!         BXD here
          ...
          if (BXD .eqv. .true.) then
             if ((iGeoStep .ge. bxdEquil) .and. (bxdWait .eqv. .false.) .and. (bxdStarted .eqv. .false. )) then
                ...
                write(fdBXD,bxdFormat)"bxdOn", bxdLow, bxdUp
             end if
          end if 
             if (bxdBounded .eqv. .true.) then
                if ((energy%Etotal - sum(TS)) < bxdLow) then
                     ...
                     write(fdBXD,bxdFormat)"Lower", bxdLow, iGeoStep
                     write(fdBXD,bxdFormat2)"BoxNumb", currentBox
                     DOScounts(currentBox,1) = DOScounts(currentBox,1) + 1
                     if (currentBox == 0) then
                        write(fdBXD,bxdFormat2) "Rev", DOSiterations
                        DOSreverse = .false.
                        if ((DOSiterations == maxDOSiterations) .and. (DOS .eqv. .true.)) then
                           tGeomEnd = .true.
                           do idos=1,bxdBoxes
                           write (fdDOS,'(I10,I10)'), DOScounts(idos,1),DOScounts(idos,2)
                           enddo
                           close(fdDOS)
                     ...
                     write(fdBXD,bxdFormat2)"BoxNumb", currentBox
                     DOScounts(currentBox,2) = DOScounts(currentBox,2) + 1
                     if (COMreverse .eqv. .true.) then
                        call bxdInvert(pMDIntegrator,movedMass,constraint)
                     end if
                     iGeoStep = iGeoStep - 1
                     ...
                     if ((boxCount .ge. bxdBoxes) .and. (DOS .eqv. .false.)) then
                        bxdUp = bxdLow + 11.0
...
!         If we are stuck at a boundary
          ...
          call next(pMDIntegrator, movedAccel ,new3Coord, movedVelo, revIdx, rxnCountDown, reversal)
