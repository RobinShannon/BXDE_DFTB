...
contains
  ...
  subroutine parseHSDInput(input)
    ...
    call readHamiltonian(tmp, input%ctrl, input%geom, input%slako)
...
!   BXD
    ...
    ctrl%bxdEvents = 50
    ctrl%bxdEquil = 100
    ctrl%bxdLower = 0.0_dp
    ctrl%bxdUpper = 0.001000_dp
    ctrl%DOSiter = 10000
    ctrl%DOS = .false.
    ctrl%DOSbox = 1
    ctrl%maxHits = 50
...
!      end if
    ...
    case ("velocityverlet")
      ...
      call getChildValue(node, "BXD", ctrl%BXD, .false.)
      call getChildValue(node, "COMbxd", ctrl%COMbxd, .false.)
      call getChildValue(node, "bxdBoxes", ctrl%bxdBoxes, 10)
      call getChildValue(node, "bxdEvents", ctrl%bxdEvents, 50)
      call getChildValue(node, "bxdEquil", ctrl%bxdEquil, 100)
      call getChildValue(node, "bxdLower", ctrl%bxdLower, 0.0_dp)
      call getChildValue(node, "bxdUpper", ctrl%bxdUpper, 0.01_dp)
      call getChildValue(node, "DOS", ctrl%DOS, .false.)
      call getChildValue(node, "DOSiter", ctrl%DOSiter, 1000)
      call getChildValue(node, "DOSbox", ctrl%DOSbox, 1)
      call getChildValue(node, "maxHits", ctrl%maxHits, 100)
      call getChildValue(node, "adaptive", ctrl%adaptive, .false.)
      call getChildValue(node, "adaptiveLimit", ctrl%adaptiveLimit, 100)
      call getChildValue(node, "maxE", ctrl%maxE, 100.0_dp)


      call getChildValue(node, "revAll", ctrl%revAll, .false.)
      call getChildValue(node, "revDis", ctrl%revDis, .false.)
      call getChildValue(node, "rxnWindow", ctrl%rxnWindow, 20)
      call getChildValue(node, "EndOnReaction", ctrl%endOnReact, .false.)
      call getChildValue(node, "restrain", ctrl%restrain, .false.)
      call getChildValue(node, "minCOM", ctrl%minCOM, 5.0_dp)
      call getChildValue(node, "COMindex", ctrl%COMindex, 0)
      call getChildValue(node, "bxdWait", ctrl%bxdWait, .false.)
      call getChildValue(node, "restrainWait", ctrl%restrainWait, 0)
      call getChildValue(node, "friction", ctrl%fric, 0.00_dp)
      call getChildValue(node, "tempK", ctrl%tempK, 0)
